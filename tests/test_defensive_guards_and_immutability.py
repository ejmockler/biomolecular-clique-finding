"""Tests for defensive guards and immutability across modules.

Covers inter-gene correlation denominator, specificity NaN guard, KDE zero
filter, multi-pass outlier contamination, ddof consistency, narrowed exception
handling, VSN reference_sample removal, all-NaN median normalization,
sample_metadata defensive copy, and permutation dict lookup.
"""
...
class TestInterGeneCorrelationDenominator:
    """rho_bar should divide by n_valid, not n_off_diag."""
...
class TestSpecificityNullCorr:
    """NaN from np.corrcoef on constant inputs must not propagate."""
...
class TestKDEZeroFilter:
    """KDE detector should not exclude zero values."""
...
class TestMultiPassContamination:
    """Pass 2 residual model must mask pass-1 outliers."""
...
class TestDdofConsistency:
    """Null distribution std should use ddof=1."""
...
class TestNarrowedException:
    """SexClassifier CV should catch specific exceptions."""
...
class TestVsnReferenceRemoved:
    """reference_sample param removed from vsn_normalization."""
...
class TestAllNanMedianNorm:
    """median_normalization should not crash on all-NaN samples."""
...
class TestSampleMetadataCopy:
    """sample_metadata property must return a defensive copy."""
...
class TestPermutationDictLookup:
    """Null distribution lookup should be O(1) dict, not O(N) filter."""
