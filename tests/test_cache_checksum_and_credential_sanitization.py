"""Tests for cache integrity checksums, credential sanitization, and batch gene resolution.

Validates:
- SHA-256 sidecar checksums for correlation matrix caches
- Neo4j credential sanitization in error messages
- Batch gene resolution using local HGNC symbol map
"""
...
class TestBatchGeneResolution:
    ...
    def test_get_regulator_modules_uses_batch_resolution(self):
        """get_regulator_modules uses batch resolution for gene names."""
        from cliquefinder.knowledge import cogex as cogex_mod
