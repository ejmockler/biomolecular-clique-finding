"""
Tests for Wave 1 audit remediation: STAT-CORE-1 (VSN GPU NaN) and STAT-CORE-10 (Li-Ji M_eff).

STAT-CORE-1: The MLX GPU VSN path incorrectly computed the reference array when data
contained NaN values. The old code used mx.mean(mx.where(isnan, 0, y)) which divides
by the total column count including NaN positions, producing a diluted reference.
The fix computes valid counts per feature and divides correctly (nanmean equivalent).

STAT-CORE-10: The Li-Ji M_eff formula used len(eigenvalues) after filtering near-zero
eigenvalues, instead of the original test count M. Additionally, variance was computed
only over filtered eigenvalues instead of zero-padded to length M per the paper.
"""
...
# ---------------------------------------------------------------------------
# STAT-CORE-1: VSN GPU NaN handling
# ---------------------------------------------------------------------------
...
class TestVSNGpuNanHandling:
    ...
    def test_reference_matches_cpu_with_nan(self):
        ...
        This is the core regression test for STAT-CORE-1. With 20% missing data,
        the old code would compute sum_of_valid / n_total instead of
        sum_of_valid / n_valid, producing systematically lower reference values.
        """
...
class TestLiJiMeff:
    ...
    def test_meff_uses_original_m_not_filtered(self):
        ...
        This is the core regression test for STAT-CORE-10. We construct a 10x10
        correlation matrix with 3 near-zero eigenvalues. After filtering, only 7
        eigenvalues remain. The old code would use M=7; the fix uses M=10.
        """
