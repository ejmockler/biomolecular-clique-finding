"""
Tests for Audit II Wave 3 GPU precision fixes:

GPU-3: Float32 catastrophic cancellation in OLS residuals
    - RSS computed via algebraic identity Y'Y - beta'X'Y instead of sum((Y - Y_pred)^2)
    - Verifies sigma2 accuracy when R^2 ~ 0.999 (near-perfect fit)
    - Verifies negative RSS is floored to 0 with warning

GPU-4: Satterthwaite df always uses float64
    - MLX path removed for tiny covariance matrices
    - Verifies float64 precision for quadratic form

GPU-5: Median polish float64 accumulation
    - Row/column effects accumulated on CPU in float64
    - GPU still used for median computation
    - Verifies accuracy with values that accumulate rounding error in float32

GPU-7: Hard MLX requirement removed
    - run_permutation_test_gpu falls back to CPU with warning
    - No ImportError when MLX unavailable
"""
...
class TestGPU3RSSAlgebraicIdentity:
    ...
    def _compute_sigma2_algebraic(self, prob):
        """GPU-3 fix: algebraic identity in float64."""
        Y = prob["Y"]
        X = prob["X"]
        XtX_inv = prob["XtX_inv"]
    ...
    def test_gpu_path_accurate_high_r2(self):
        """The GPU path (with GPU-3 fix) produces accurate t-stats for high R^2.

        The key improvement: beta is computed on GPU (float32) then RSS is
        computed on CPU (float64) via the algebraic identity. This avoids
        the catastrophic cancellation of Y - Y_pred in float32.
        ...
        # GPU-3 fix: RSS is now computed in float64 via algebraic identity,
...
# =============================================================================
# GPU-4: Satterthwaite df Always Float64
# =============================================================================


class TestGPU4SatterthwaiteFloat64:
    ...
    def test_mlx_path_removed(self):
        ...
        assert len(matmul_called) == 0, (
            "mx.matmul should not be called in satterthwaite_df (GPU-4 fix)"
        )
...
class TestGPU5MedianPolishFloat64Accumulation:
    ...
    def test_accumulation_matches_cpu_reference(self):
        ...
        # With GPU-5 fix (float64 accumulators), GPU should match CPU closely
    ...
    def test_many_iterations_no_drift(self):
        ...
        # Even with many iterations, GPU-5 float64 accumulators should prevent drift
...
# =============================================================================
# GPU-7: Graceful MLX Fallback
# =============================================================================


class TestGPU7GracefulFallback:
