"""Tests for Audit XIV Wave 1 fixes.

XIV-1: MEAN50 MIXED signed weights — np.abs(w) * np.abs(z) for MIXED.
XIV-2: Pass 2 outlier NaN vulnerability — np.nanmedian for residual stats.
"""
...
class TestMean50MixedAbsoluteWeights:
    """XIV-1: MEAN50 MIXED must use |w| so negative weights don't cancel."""
...
class TestPass2OutlierNaNResilience:
    """XIV-2: Pass 2 must detect outliers even when some cells are NaN."""
