"""Tests for missing value handling and boundary condition guards.

Covers censored quantile normalization direction, negative controls index space,
subject_col/use_mixed_model warnings, use_eb deprecation, negative CV guard,
zero-IQR adjusted boxplot fences, iloc boolean mask compatibility, and NaN
variance guard for cost matrices.
"""
...
class TestCensoredQuantileDirection:
    """Censored quantile normalization must map to UPPER quantiles."""
...
class TestUseEbDeprecated:
    """use_eb=False must emit DeprecationWarning."""
...
class TestNegativeCVFix:
    """CV must be non-negative even when medians are negative."""
...
class TestZeroIQRFences:
    """Zero-IQR adjusted boxplot should not flag everything."""
...
class TestLocBooleanMask:
    """iloc with boolean mask replaced by loc for pandas compatibility."""
...
class TestNaNVarianceGuard:
    """NaN variances from ddof=1 must not poison cost matrix."""
