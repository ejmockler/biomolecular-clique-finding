...
All **3,264 measured features** are attempted as anchors in the bounded, measured-only gradient analysis. Of those, **3,117 yield valid two-shell gradients**. The remaining 147 carry one of two recorded reasons: **137** are recorded as `DisconnectedFeature`, and **10** fall below the implemented guardrail requiring at least 10 measurable genes in the neighborhood.

The 137 are worth splitting, because the recorded reason flattens two different things. Six rows — `1/iRT_protein`, P30042, P51784, Q58FG1, Q9BRJ2, Q9Y4D8 — never resolved to a gene symbol, so they were never nodes in the extracted graph and their reachability was never actually evaluated. The other 131 did enter the graph, carry a recorded full-INDRA degree, and are genuinely isolated within the measured-only depth-2 subgraph. The retained metadata cannot tell these apart on its own: the `unmatched` field exists to record non-resolution but is populated only for features that already resolved, so it is empty in every run. None of the 147 is a missing-intensity exclusion — the abundance matrix is complete. The iRT standard is one of the six unresolved rows.
...
- **The walk is confined to the measured feature set.** The intent is that a path never routes through an unmeasured intermediate, so that a shell at distance $h$ contains only proteins we actually have $|t|$ values for — keeping every shell average an average over real measurements and preventing phantom nodes from bending the distance metric.

That second property deserves scrutiny, because it holds only as firmly as the identifier used to define "measured". In the analysis reported here each measured row was represented by *every* gene symbol and alias a live lookup returned for it, and graph nodes were matched on their bare name. Names are not unique to entities. When an alias of a measured protein is also the official symbol of a different, unmeasured gene — or of a protein family, or even a chemical — that other entity enters the graph wearing the measured row's identity, and brings all of its edges with it.

The clearest example: `AR` is an alias of the measured protein AKR1B1. The androgen receptor is not in this proteome, but it *is* in the knowledge graph under the name `AR`, carrying 14,818 regulatory relationships. It was therefore admitted as AKR1B1, which is why that row has a first shell of 821 measured proteins when the dataset median is 17 — and why two-hop pairs bridged only by that row were, in fact, routed through an unmeasured protein. Across the proteome this touched 119 of 3,258 graph-resolved rows and 96 of the 1,407 confirmatory anchors; most cases are protein-family nodes (`AKT` standing in for the AKT family) rather than gene homonyms. Representing each feature by a single namespaced gene identifier collapses each row to exactly one entity and removes the ambiguity; the results reported below predate that and carry this caveat.
...
- At depth 2, the slope compares an anchor's direct regulatory partners (ring 1) with two-hop regulatory partners reached through a measured intermediate (ring 2). This is a proximity question in the topological sense — two undirected hops — not a directional cascade.
- "Local" here means *two hops*, and it is worth being precise about how much of the graph that already covers. In the analyzed measured-only network the ring-1 shell has a median of 17 measured proteins but the ring-2 shell has a median of about 1,880 — a median 60% of the anchor's entire connected component, rising to 76% among the 1,407 confirmatory anchors. The depth-2 slope is therefore already contrasting a small direct neighborhood against most of the reachable component.

So the bound is not what keeps the statistic away from a whole-component comparison; that comparison is present at depth 2. Removing the bound raises component coverage from roughly 61% to 99% — it deepens an existing contrast rather than introducing a new one. The reason to prefer the bounded statistic is empirical, and the unbounded rerun below supplies it: the fixed-term pattern attenuates from 8/6/0 to 6/0/0 when the bound is removed.
...
For each anchor, remove that anchor from the label pool. Assign every remaining measured feature a full-INDRA count of incident regulatory relationships, taking the maximum over its resolved aliases. This is not the count of unique measured hop-1 neighbors, and its query uses the statement-type filter without reapplying the shell extraction's explicit evidence-count predicate. It is also not always the feature's *own* count: the query is keyed on the bare INDRA entity name, so an alias shared with another entity contributes that entity's degree instead. For 119 of the 3,258 degree-carrying features (96 of the 1,407 robust anchors) the assigned degree exceeds that of the feature's own gene — most often a protein-family node (AKT1 gets `fplx:AKT`'s 35,596 in place of its own 9,070), sometimes another gene entirely (AKR1B1 gets the androgen receptor's 14,818 in place of 2,120). Since degree only decides which 100-wide rank block a label sits in, and the pathway ranking never consumes these p-values, the effect is bounded to this diagnostic. Sort features by degree, split the ordering into consecutive rank blocks of 100, and shuffle $|t|$ within each block. This approximately retains degree location; it is not exact degree matching, and “hub swaps only with hub” would overstate it. For $B=999$ permutations producing null slopes $\{\hat m_a^{(b)}\}$, the one-sided decay p-value is
