...
Outputs:
- output/landscape_proteome/manifest.yaml
- output/landscape_proteome/distances.npz + distances.meta.json
- output/landscape_proteome/result.json

After this completes, analyze with:
    from cliquefinder.panels import LandscapeResult, analyze_landscape
    result = LandscapeResult.load_json("output/landscape_proteome/result.json")
...
def main() -> None:
    ...
    out_dir = ROOT / "output/landscape_proteome"
    ...
    design = LandscapeDesign(
        ...
        covariates=("Sex",),
        description=(
            "Wave 24g full proteome — every measured protein as seed, "
            "C9_vs_SPORADIC, regulatory edges, h2"
        ),
    )
    ...
    log.info("max_hops=%d, n_permutations=%d, covariates=%s",
