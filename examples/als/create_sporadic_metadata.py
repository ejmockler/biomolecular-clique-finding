#!/usr/bin/env python3
"""
AnswerALS Sporadic Metadata Creation Example
=============================================

This is an AnswerALS-specific example demonstrating how to filter metadata
to identify sporadic ALS cases (excluding familial/genetic variants).

Dataset: AnswerALS Portal Metadata
- Input: aals_dataportal_datatable.csv
- Output: data/sporadic_metadata.csv

Filtering Logic:
1. Identifies Familial ALS based on mutation data:
   - ClinReport_Mutations_Details is not NaN/Empty
   - HasVariant is not NaN/Empty (and not just "None")
2. Relabels Familial ALS samples to exclude from CASE group
3. Keeps only Sporadic ALS (no known mutations)

This is specific to AnswerALS data schema and should be adapted for other
datasets with different metadata structures.
...
def create_sporadic_metadata():
    """
    Create sporadic ALS metadata by filtering out familial cases.

    AnswerALS-specific logic for identifying familial vs sporadic ALS.
    """
    ...
    if not Path(input_path).exists():
        ...
        print("\nThis script expects AnswerALS portal metadata file.")
        print("Update the input_path variable to match your data location.")
        return
